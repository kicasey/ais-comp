...
public class ResumeController : ControllerBase
{
    ...
    /// <summary>Get current user's employee profile and resume status (for employee role).</summary>
    [HttpGet("profile")]
    ...
    /// <summary>Upload or replace the single resume for the current user's employee profile.</summary>
    [HttpPost("upload")]
    public async Task<IActionResult> UploadResume([FromForm] IFormFile resume)
    {
        ...
        if (user == null) return Unauthorized();
        if (user.Role != "employee")
            return BadRequest(new { message = "Only employees can upload a resume to their profile." });
        ...
        {
            var oldPath = Path.Combine(_environment.ContentRootPath, "Uploads", profile.ResumeFilePath);
